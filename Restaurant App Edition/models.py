...
class User:
    def __init__(self, email):
        ...
        self.address = None
        # self.fav_res = None
        # self.fst = Restaurant.sell_food(self)
    ...
    def edit_passw(self, oldPass, newPassw) -> bool:
        if str(self.passw) != str(oldPass):
            print(self.passw,oldPass)
...
class Restaurant:
    def __init__(self, name, address,catagory):
        self.name = name
        self.address = address
        self.uid =uuid.uuid4()                    #uuid.uuid4()
        self.daily_menu = []
        self.rate = 0
    ...
    def find_food(self, food_name, count=1):
        for item in self.daily_menu:
            if item[0] == food_name and item[1] >= count:
                return item
        return False
